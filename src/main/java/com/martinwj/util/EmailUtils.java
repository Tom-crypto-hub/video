...
public class EmailUtils {
    ...
    public static void sendMessage(HttpServletRequest request, String username, String code, String email) {
        ...
        Send("wangjingwj666@163.com", "wangjingwj666", email, "备注", "个人音乐网站", "个人音乐网站激活", mailText);
    }

    public static void sendMessage(HttpServletRequest request, String userToken, User user) {
        String url  = "http://" + request.getServerName() + ":" + request.getServerPort() + "/" + request.getContextPath() + "/portal/user/validate_email.json?token=" + userToken;
        String mailText = "<html><head></head><body><h1>这是一封激活邮件,激活请点击以下链接</h1><h3><a href='" + url +
                "'>" + url + "</a></h3></body></html>";

        System.out.println(mailText);
        Send("wangjingwj666@163.com", "wangjingwj666", user.getEmail(), "备注", "马丁动漫网", "马丁动漫网站账号激活", mailText);
    }
}
