...
import com.martinwj.entity.media.ImageMetaInfo;
import com.martinwj.entity.media.VideoMetaInfo;
import com.martinwj.entity.media.gif.AnimatedGifEncoder;
...
/**
 * @ClassName: MediaUtil
 * @Description: TODO 基于FFmpeg内核来编解码音视频信息；
...
public class MediaUtil {
    ...
    public static void main(String[] args) {
        File videoFile = new File("D:\\UE4教程\\UE4游戏动漫制作全能系统特训营第三期\\第1周：ue4基础-BSP原型\\1-1ue4安装.mp4");
        Integer width = getVideoMetaInfo(videoFile).getWidth();
        Integer height = getVideoMetaInfo(videoFile).getHeight();
        System.out.println(width + " : " + height);
//        cutVideoFrame(videoFile,
//                new File("D:\\temp\\3.png"), new Time(0, 1, 50), width, height);
        cutVideoFrame(videoFile,
                "D:\\temp\\", new Time(0, 2, 1), width, height, 5, true);
        List<String> allFile = FileUtil.getAllFile("D:\\temp\\", ".jpg");
        createGifImage(allFile.toArray(new String[allFile.size()]), "D:\\temp\\1.gif", 3);
    }
    ...
    private static void cutVideoFrame(File videoFile, String path, Time time, int width, int height, int timeLength, boolean isContinuty) {
        ...
        try {
            ...
            if (isContinuty) {
                commond.add(path + File.separator + "foo-%03d.jpg");
            } else {
                commond.add(path);
            }
